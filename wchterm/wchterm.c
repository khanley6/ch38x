...
struct wch_term_conf 
{
	int				fd;				// device node to open
	struct 			termios	t;		// termio config

	int				open_flag;		// open flag, see wchterm.h
	int     		termio_flag;	// termio flag, see wchterm.h
	int     		mode_flag;		// mode flag, see wchterm.h
...
static struct wch_term_conf	term;
...
{
	// clear struct
	memset(&term, 0, sizeof(struct wch_term_conf));
...
static void mainFunction(int select)
{
	...
	while (1)
	{
		...
		box(mesg_win, 0, 0);
		mvwprintw(mesg_win,2,67,"Ver:%s",WCHTERMVERSION);	
		box(term_win, 0, 0);
		wrefresh(mesg_win);
		wrefresh(term_win);
...
static void *sentTerm(void)
{
	...
	while (1)
	{	
		write(term.fd, "0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVW", 40);
		term.count++;
